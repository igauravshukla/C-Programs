...
Snow Howler is the librarian at the central library of the city of HuskyLand. He must handle requests which come in the
following forms:
...
Snow Howler has got an assistant, Oshie, provided by the Department of Education. Although inexperienced, Oshie can handle all
of the queries of types 2 and 3.
	...
	while (total_number_of_queries--)
	{
		...
		if (type_of_query == 1)
		{
			...
			*(total_number_of_books+x) += 1;	// total_number_of_books[x] += 1;
			...
			/* to add the books in shelf without wasting memory, that is allocating memory according to the need, as number of
				books in a shelf is not fixed.
			*/
			*(total_number_of_pages+x) = realloc(*(total_number_of_pages+x), *(total_number_of_books+x)*sizeof(int));
			
			*(*(total_number_of_pages+x)+*(total_number_of_books+x)-1)=y;//total_number_of_pages[x][total_number_of_books[x]-1]=y;
		}
